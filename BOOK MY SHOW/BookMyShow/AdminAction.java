...
import BookMyShow.Actions.AdminActions;
...
public class AdminAction implements AdminActions {
    ...
    public  void operations(Admin currentAdmin) {
        while (true) {
            System.out.println("\n" + "---------------------------------------------------------" + "\n");
            ...
            switch (choice) {
                case "1":
                    addMovie();// based on the choice call the function (1)
                    break;
                case "2":
                    viewallmovies();// based on the choice call the function (2)
                    break;
                case "3":
                    addTheatre();// based on the choice call the function (3)
                    break;
                case "4":
                    viewalltheatre();// based on the choice call the function (4)
                    break;
                case "5":
                    return;// exit
                default:
    ...
    public  void addMovie() {
        ...
        LocalDate dateStart = LocalDate.parse(scanner.nextLine(), BookMyShow.getUtilities().getFormatter());// LocalDate class used and formatted the date as in utilities
        boolean checkTheatreinThatlocation = false;// number of theatre in that locations

        if(dateStart.isBefore(BookMyShow.getUtilities().getToday())){
        ...
        show: while(true) {
            ...
            LocalTime choiceStartTime = LocalTime.parse(scanner.nextLine(), BookMyShow.getUtilities().getTimeformatter());// Using localTime store the time and format is (HH:mm)
    ...
    @Override
    public  void viewallmovies() {//to print all movies that are added
    ...
    public  void addTheatre() {
        ...
        screen: while (screenCount <= numberOfScreens) {
            ...
            var seatsAndGrid = BookMyShow.getUtilities().generateGrid(numberOfSeats, grid);//get pattern from utilities
    ...
    @Override
    public void viewalltheatre() {//print all the theatres that are added
