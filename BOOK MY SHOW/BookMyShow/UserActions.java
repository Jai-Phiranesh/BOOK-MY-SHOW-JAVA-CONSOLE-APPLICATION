...
import BookMyShow.Actions.UserAction;

public class UserActions implements UserAction{
    public static Scanner scanner = new Scanner(System.in);
    @Override
    public  Account login() {// perform the login action of the user
    ...
    public  void register() {// if no account the function is called
    ...
    public  void operations(User currentUser) {


        displayAllMovies(currentUser, BookMyShow.getUtilities().getToday());// very bigning of the login show the movies
        ...
        while (true) {
            ...
            switch (choice) {
                ...
                case "1":// based on the choice call the function (1)
                    ...
                    displayAllMovies(currentUser, BookMyShow.getUtilities().getToday());// after the change in the location
                                                                                    // show the movies
                    break;
                case "2":// based on the choice call the function (2)
                    ...
                    displayAllMovies(currentUser, dateUpdate);// after the change in the date show the  movies
                ...
                case "4":// based on the choice call the function (4)
                    displayAllMovies(currentUser, BookMyShow.getUtilities().getToday());
                    break;
    ...
    public  void displayAllMovies(User currentuser, LocalDate today) {// display movies based on users location and date
    ...
    public  void show_movie_details(ArrayList<Movies> movieavailable, String moviechoice, User currenUser) {
       ...
       show: while(true){
        System.out.print("Enter the Show Start Time example:(09:00):");
         ...
         BookMyShow.getUtilities().getTimeformatter());// get the show start time
    ...
    public static LocalDate changeDate(User currentuserob) {// tp change date
        ...
        LocalDate dateupdate = LocalDate.parse(scanner.nextLine(), BookMyShow.getUtilities().getFormatter());
        if(dateupdate.isBefore(BookMyShow.getUtilities().getToday())){
